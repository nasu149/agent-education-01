...
ハンズオン開始後は、次の完成版・実験版ブランチの Agent 実装を参照しません。

```text
student-complete
experiment/db-connection-exhaustion
```

README や講師が指定した説明資料は参照可能です。

目的は暗記した正解コードの再現ではなく、チームで Agent の設計を考えることです。
...
- Gemini API の 429 / 5xx / transport error など、Agent 設計と無関係な明確な基盤エラーは講師判断で1回だけ再試行可
- 誤診や余分な Tool 選択など、通常の LLM の揺らぎは再試行理由にしない
